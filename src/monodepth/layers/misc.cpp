...
namespace monodepth {
  namespace layers {
    ...
    torch::Tensor get_translation_matrix(torch::Tensor translation){
      torch::Tensor T = torch::zeros(translation.shape[0], 4, 4).to(device = translation.device);
      torch::Tensor t = translation.contiguous().view(-1, 3, 1);
    ...
    torch::Tensor rot_from_axisangle(torch::Tensor axis){
      ...
      torch::Tensor rot = torch.zeros((axis.shape[0], 4, 4)).to(device=axis.device);
    ...
    torch::Tensor transformation_from_parameters(torch::Tensor axisangle, torch::Tensor translation, bool invert){
      ...
      if(invert){
        torch::Tensor M = at::matmul(R, T);
      }else{
        torch::Tensor M = at::matmul(T, R);
      }
    ...
    torch::Tensor Project3DImpl::forward(torch::Tensor points, torch::Tensor K, torch::Tensor T){
      torch::Tensor P = at::matmul(K, T)[:,:3,:];
