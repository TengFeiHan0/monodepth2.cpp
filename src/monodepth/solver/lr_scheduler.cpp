...
double WarmupMultiStepLR::get_lr(){
  ...
  //std::cout << pow(_LRScheduler<torch::optim::SGD>::base_lr * warmup_factor * gamma_, monodepth::utils::bisect_right(milestones_, _LRScheduler<torch::optim::SGD>::last_epoch_)) << "\n";
  return _LRScheduler<torch::optim::SGD>::base_lr * warmup_factor * pow(gamma_, monodepth::utils::bisect_right(milestones_, _LRScheduler<torch::optim::SGD>::last_epoch_)); 
}
